...
> 仓库提供 private catalog 与相关代码（private HMAC v2 落于 migration `0031`，当前工作树连续为 `0001..0039`），不代表任何现有 production 已启用。2026-07-22 的本机 live production 仍停留在 migration 26，且没有挂载该 catalog；受控 v26→v39 迁移必须满足无活动租约/device rebind/key-state/预算、0037 认证/ACL、0038 速度字段与 0039 API Key 最小权限前置条件。当前 fresh-v39 已以每 binary 独立数据库完成 `49/49`、无 skip；production 配置与真实模型验收完成前，仍不能对该部署声称启用 private Hidden Benchmark。

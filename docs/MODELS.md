...
同一用户可以按端口同时运行多个本地受管实例：默认端口 `8080` 保留兼容状态文件，其他端口使用独立状态、日志和运行目录。`--replace` 只停止并替换同一端口上的实例，不影响其他端口；`serve status/stop --port` 精确管理指定端口。贡献节点当前只发布默认 `8080` 实例，额外端口用于本地独立调用，不能冒充额外共享容量。
...
2026-07-22 的实时审计中，低并发全目录运行解析成功 61 项，另外 4 项被 HF `429` 限流；随后逐项重试也全部解析成功，合计 65/65 均找到了带大小和 LFS SHA-256 的完整主 GGUF 清单。Qwen3-0.6B 另做了真实 Range 探测，读取恰好 65,536 bytes 后中止，`persisted=false`，未保存完整模型。这些证据不代表 65 个模型均已下载，也不代表超大模型能在当前机器运行。

...
| Windows 安装：与 Unix 对齐的合同、checksum、zip 精确清单、原子覆盖更新、拒绝重解析链、用户 PATH | `scripts/install.ps1`、`.cargo/config.toml`、`scripts/verify-windows-self-contained.ps1` | PowerShell 7.5 解析通过；Windows workflow 已配置用户 PATH/当前进程 PATH、裸 `mindone`、`-NoModifyPath` 以及空项/空格/尾分隔符无损保留合同。`cargo-xwin 0.23.0` 已完成 x86_64 MSVC 全工作区 check/严格 Clippy/17 MiB release PE；导入审计确认静态 CRT、无外部 VC/UCRT DLL，Job Object API 完整，原生 CI/Release 会用 `dumpbin` 复验 | 交叉构建完成；安装、TUI、Credential Manager 与 Job Object 生命周期仍待 Windows Actions/真机 |
| Windows 卸载：默认保留数据，`-PurgeData` 才删除；精确清理安装目录 PATH；拒绝 root/user/reparse/非规范/外部项/缺 CLI 的服务状态 | `scripts/uninstall.ps1` | Windows CI 覆盖保留、彻底删除、其他 PATH 项逐字节恢复、服务安全停止与拒绝边界 | 待外部验证（Windows Actions） |

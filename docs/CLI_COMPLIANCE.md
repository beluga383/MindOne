...
| 官方 llama.cpp 审计发行物真实下载并执行 | `crates/mindone-engine/tests/real_install.rs`、`scripts/e2e-test.sh` | 历史 debug CPU-only E2E 从官方发行渠道取得并真实执行 llama.cpp b10064，使用 Qwen3-0.6B-Q4_0 GGUF 完成动态推理；当前隔离轮又完成整包下载、验证和非默认端口健康启动 | macOS arm64 当前四槽/统一 KV argv 已到 health；旧脚本随后检查错端口而停止，当前 Standard job 全程与其他 OS/arch、GPU backend 待流水线复验 |

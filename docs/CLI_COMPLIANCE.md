...
| 远程单命令安装后直接进入 TUI | Unix `install.sh --launch`、Windows `install.ps1 -Launch`；交互式终端启动已安装 CLI，非交互/CI 安全降级为真实帮助页；默认只写当前用户受管 PATH，可显式关闭 | Unix release smoke 验证受管 PATH 唯一性、新 shell 裸 `mindone` 与卸载清理；Windows Actions 脚本验证用户 PATH/当前进程 PATH 与裸命令 | 代码与 Unix 本地证据完成；公开仓库/Release/raw URL 当前为 404，Windows runner 尚未实际触发 |
...
| `share publish` | 必填 `--model`；可选 `--alias`、逗号分隔 `--tags` | 注册节点/实例、首次心跳后上线；`10`/`20`/`21` | 完成 |
...
| Token/设备私钥只进系统凭证库 | `crates/mindone-cli/src/vault.rs`、`crates/mindone-common/src/secret.rs` | `vault::tests::{memory_store_round_trip_and_clear_are_scoped,debug_output_redacts_tokens_and_x25519_private_key}`；共享业务单元测试统一用仅 `cfg(test)` 可见的内存 SecretStore，不再依赖 Linux 桌面 DBus | 完成（本地自动测试）；production 构建仍只能创建系统凭证库，各平台真实 Keychain/Secret Service smoke 待外部验证 |
...
| 官方 llama.cpp 审计发行物真实下载并执行 | `crates/mindone-engine/tests/real_install.rs`、`scripts/e2e-test.sh` | 历史 debug CPU-only E2E 从官方发行渠道取得并真实执行 llama.cpp b10064，使用 Qwen3-0.6B-Q4_0 GGUF 完成健康检查与动态推理 | macOS arm64 历史本机实测；本轮四槽/统一 KV argv 与其他 OS/arch、GPU backend 待复验 |
...
| CPU-only 是类型化受管策略，附加参数和环境不能覆盖 | `crates/mindone-cli/src/serve.rs`、`crates/mindone-engine/src/process.rs` | `ServeRequest.cpu_only` 单独传递；引擎统一注入 device/GPU layer/KV-op offload 禁用参数、清除对应环境覆盖，并对冲突附加参数失败关闭；定向测试与历史 CPU-only 真实 E2E 通过 | 类型化边界完成；四槽真实小模型 E2E 待复验 |
| 沙盒只读精确模型、引擎执行路径、runtime 写目录 | `crates/mindone-sandbox/src/launch.rs` | launch path/canonicalization/Seatbelt profile 单元测试；Linux/macOS 真实 gate 已配置为 ignored + env-gated | 部分完成；当前 macOS 真实 gate 通过，Linux CI 与 Windows 平台能力仍待外部验证 |
...
| Linux namespaces + no-new-privs + seccomp + Landlock，缺能力 fail closed | `crates/mindone-sandbox/src/launch.rs`、`crates/mindone-sandbox/src/linux_supervisor.rs`、`crates/mindone-sandbox/src/capability.rs`、`crates/mindone-cli/tests/linux_sandbox.rs` | 测试为 `#[ignore]` 且要求 `MINDONE_REAL_LINUX_SANDBOX_TEST=1`；`.github/workflows/ci.yml::linux-sandbox-gate` 安装可信 bubblewrap、只临时调整 runner sysctl，并显式验证允许目标/拒绝 sibling | 当前轮未运行；待外部验证（Actions 未运行） |
...
| 命令/合同 | 真实实现 | 自动化证据 | 状态 |
|---|---|---|---|
| `share publish` 注册节点/硬件、发布模型、启动 worker/心跳 | `crates/mindone-cli/src/share.rs`、`crates/mindone-coordinator/src/routes/nodes_models.rs` | share/PostgreSQL 测试；最终 E2E 得到真实 worker PID、首次心跳与在线状态 | 完成（本机实测） |
...
| Unix 卸载：默认保留数据，显式 purge，父链/目标 symlink、宽路径、孤儿服务状态与非 MindOne 文件 fail closed | `scripts/uninstall.sh` | release smoke 验证默认卸载保留数据、移除受管 PATH 块与 `--purge-data`；负向门禁有定向证据 | 三个 Unix 用户态目标通过；Linux 真实沙盒与 Windows Actions 仍待外部验证 |
| Windows 安装：与 Unix 对齐的合同、checksum、zip 精确清单、原子覆盖更新、拒绝重解析链、用户 PATH | `scripts/install.ps1`、`.cargo/config.toml`、`scripts/verify-windows-self-contained.ps1` | PowerShell 7.5 解析通过；Windows workflow 已配置用户 PATH/当前进程 PATH、裸 `mindone` 与 `-NoModifyPath` 合同。`cargo-xwin 0.23.0` 已完成 x86_64 MSVC 全工作区 check/严格 Clippy/17 MiB release PE；导入审计确认静态 CRT、无外部 VC/UCRT DLL，Job Object API 完整，原生 CI/Release 会用 `dumpbin` 复验 | 交叉构建完成；安装、TUI、Credential Manager 与 Job Object 生命周期仍待 Windows Actions/真机 |
| Windows 卸载：默认保留数据，`-PurgeData` 才删除；精确清理安装目录 PATH；拒绝 root/user/reparse/非规范/外部项/缺 CLI 的服务状态 | `scripts/uninstall.ps1` | Windows CI 覆盖保留、彻底删除、PATH 恢复、服务安全停止与拒绝边界 | 待外部验证（Windows Actions） |
| 多平台构建、发行 checksum、SBOM/provenance、可选平台签名 | `.github/workflows/ci.yml`、`.github/workflows/release.yml`、`.github/workflows/security.yml`、`deny.toml` | Release 先验证标签提交可达 `main`，再复用同一提交的完整 CI（含 Linux/macOS 沙盒与 Unix/Windows 安装门禁）；Windows 包还必须通过静态 CRT/Job Object `dumpbin` 门禁。workflow YAML/固定 action SHA 静态检查与 cargo-deny 四类策略本地通过；真实 workflow 尚无链接 | 交叉 Windows release 已构建并审计；原生 workflow、上传与签名待外部验证 |
...
| 安装、版本、中文帮助、doctor、卸载无残留 | 当前 `mindone 1.0.1` 已在 macOS arm64 完成本轮 release smoke；先前 `1.0.0` 已在 Linux arm64 和 Linux x86_64 用户态完成同类 smoke，x86_64 还真实执行 `model recommend --json` 与 `api info --json` | Windows Actions 与 `1.0.1` Linux 发行流水线仍以最终外部结果为准 |
| Linux 四层沙盒与 macOS Seatbelt 真实 allow/deny | 两项真实测试均为 `#[ignore]` 并分别要求 `MINDONE_REAL_LINUX_SANDBOX_TEST=1` / `MINDONE_REAL_SEATBELT_TEST=1`；当前 macOS 已显式重跑 1/1 passed，普通 workspace test 不能把 ignored/unavailable 当通过 | macOS 当前本机通过；Linux 四层真实门禁与两个平台 Actions 待外部验证 |
| RustSec、cargo-deny、Gitleaks 与 workflow/script 静态门禁 | cargo-audit 0.22.2 本轮联网刷新到 1167 条 advisory 后为 0 vulnerability / 0 warning；`cargo deny check`、actionlint、3 个 workflow YAML 解析和当前 16 个 Shell 脚本语法检查均退出 0；Gitleaks 8.30.1 对排除构建缓存后的 237 个实际项目文件（约 4.38 MB）扫描通过 | 依赖与静态门禁已有当前本机证据；Actions 仍待外部验证 |

...
各功能行保留的是最近一次与该行为直接相关的证据；是否已经满足**当前工作树**的最终发布门禁，以第 0 节和第 13 节为准。当前源码与 live production 均为连续 `0001..0039`：`0038` 增加三档任务速度，`0039` 增加 HMAC-only 推理 API Key 及只追加审计事件。下文 fresh-v36/v37 和 workspace `556/0/5` 均是新增 0038/0039 前的历史阶段证据，不能冒充当前全量门禁。
...
| 当前 migration / fresh v39 数据库 | migration 文件连续 `0001..0039`；一次性 PostgreSQL 17 上 16 个 binary 各用独立数据库，合计 `49/49`、无 skip；持久库与 live metadata 均为 `39|1|39|t` | 独立数据库避免 `database_role` 的故意错误 HMAC commitment 污染其他 binary；live 切换另有备份、ACL 与公网证据 |
...
| Compose 静态配置门禁 | base、Cloudflare public overlay、quality operator overlay、dev 的规范化 Compose 断言全部通过，4/4；专用 connector 已以固定 digest 运行且无宿主端口 | `api.holarchic.cn` 的 origin/公网身份链已实测；不代表根域、其他 Tunnel 或 private 功能有改动 |
| 本机 production 备份、升级与切换 | v26 恢复副本真实 `26→39` 演练通过后，停写时确认 active jobs/prepared routes 为 `0|0`，新建 custom dump 203480 字节、381 项、SHA-256 `cf4bc43886ec4473ee705720a2f8e112e11c9bdd47e95b1e4a966c510398443f`；live migrator/role-init 均退出 0，metadata `39|1|39|t`，0037/0038/0039 对象、零业务行和最小 ACL 已核对 | private catalog/HMAC 预算、production billing profile、真实 SMTP 与 Windows 真机仍是独立门禁 |
...
| `doctor` | `--server-mode` 可选 | 检查客户端能力；server-mode 额外检查 DB；失败 `1`、信任降级 `31` | 完成；专用 connector、loopback origin 与公网 Cloudflare identity 已实测一致 |
...
| `engine detect` OS/CPU/RAM/GPU/Metal/CUDA | `crates/mindone-cli/src/engine.rs`、`crates/mindone-engine/src/hardware.rs` | `hardware::tests::detects_real_nonzero_host_resources`、`hardware::tests::parses_current_and_legacy_macos_metal_capability_without_false_positive` | macOS 26 / Apple M4 已按 JSON family 实测为 Metal；其他 GPU/平台准确性继续由对应原生门禁验证 |
...
| 节点无需公网入站，llama-server 不得公开 | `crates/mindone-cli/src/share.rs`、`deploy/docker-compose.yml`、`deploy/docker-compose.cloudflared.yml` | 本地 worker 只访问 loopback；公网 overlay 不发布 origin，只允许固定 IP 的专用 connector；live coordinator 与 connector 均无 published ports，PostgreSQL 只在 internal backend | 完成（live v39 网络实测） |
...
| production 数据库 owner/runtime 分离，runtime 不执行 schema migration | `crates/mindone-coordinator/src/db.rs`、`crates/mindone-coordinator/src/main.rs`、`migrations/0026_runtime_database_role.sql`、`deploy/docker-compose.yml`、`deploy/postgres-ensure-runtime-role.sh` | 历史 fresh v36/v37 门禁；当前 fresh PostgreSQL 17 上 `runtime_schema` `2/2` 与 `database_role` 已通过，并覆盖 0039 两表最小权限；live role-init 与 ACL 实查通过 | 完成（live v39 实测）；常驻 coordinator 仍只使用 `mindone_app`，不持有 owner 凭据 |
...
| 命令/合同 | 真实实现 | 自动化证据 | 状态 |
|---|---|---|---|
| 全新客户端默认官方协调器 `https://api.holarchic.cn`；独立 API 子域避免覆盖现有官网；已存在配置不覆盖；loopback 只由开发者显式设置 | `crates/mindone-common/src/config.rs`、`crates/mindone-cli/src/context.rs` | `config::tests::fresh_client_defaults_to_official_tls_origin`、配置原子 round-trip；公网 TLS/identity/CF-Ray 实测通过 | 完成（代码、自动测试与 live v39 公网实测） |
...
| PostgreSQL migrations、最小权限角色与完整 API 集成 | 当前源码连续 `0001..0039`；fresh-v39 16 个 binary 各用独立数据库，合计 `49/49`、无 skip，覆盖最小 ACL、速度档调度与 API Key/OpenAI JSON + Standard SSE 网关 E2E | live production 已为 `39|1|39|t`，且 migrator/role-init/ready 均验收通过 |
...
| private HMAC v2、预算与 terminal capability | fresh-v39 `49/49` 覆盖跨 catalog 真重叠双 `PgPool`、key-state、v2 raw-null、设备绑定与 terminal capability | production v39 未配置 key/预算/catalog，不能宣称启用 |
| Cloudflare 五项公网安全测试 | HTTPS `/health` 与 `/ready` 为 MindOne 1.0.2 且带 `CF-Ray`；匿名模型/额度接口 401；未知路径 404；数据库、origin、llama-server 与本地代理均无宿主/公网端口 | 正确 Token 的业务调用需由真实账号/API Key 验收；未伪造账户或额度 |
...
当前源码的 40 个公开叶子参数矩阵和 TUI 10 类映射已落盘并通过完整 CLI 与 workspace 门禁；HF 日常小模型部署探测只读取 64 KiB 后断开。按端口并行运行多个本地受管实例已实现并通过状态隔离/在用保护的确定性测试，没有下载第二个真实模型。当前 workspace `590/0/5`、fresh-v39 `49/49`、Unix/Windows 安装门禁、macOS Seatbelt、Linux 四层沙盒、五目标原生编译、当前四槽 CPU-only 小模型业务链、API Key 网关 JSON/SSE 数据库 E2E、v1.0.2 五平台发行、production live v39 与公网 API 子域 TLS/identity 均已有真实证据；Windows 真机交互/模型启动、外部 SMTP、Apple/Windows 平台原生签名、private 双 GGUF 与真实 TEE 仍未完成。

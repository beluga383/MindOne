...
```bash
curl -fsSL https://raw.githubusercontent.com/beluga383/MindOne/main/scripts/install.sh \
  | sh -s -- --version v1.0.1 --install-dir "$HOME/.local/bin"
```
...
```bash
cargo install --locked --git https://github.com/beluga383/MindOne --tag v1.0.1 mindone-cli
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/beluga383/MindOne/main/scripts/install.sh \
  | sh -s -- --version v1.0.1 --allow-downgrade
```

```powershell
$installer = [scriptblock]::Create((irm https://raw.githubusercontent.com/beluga383/MindOne/main/scripts/install.ps1))
& $installer -Version v1.0.1 -AllowDowngrade
```
...
全新配置默认使用官方专用 API origin `https://api.holarchic.cn`；该子域用于避免覆盖 `holarchic.cn` 的现有官网。部署方完成子域 TLS/Tunnel 前登录会明确失败，不能把官网 404 当成协调器。已有 `config.toml` 始终保留其原值，不会在升级时被覆盖；曾配置根域的客户端应显式执行 `mindone config set server.url https://api.holarchic.cn`。本地开发才改为 loopback：

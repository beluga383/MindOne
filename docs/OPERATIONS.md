...
本机 live production 已于 2026-07-23 受控升级为 `0001..0039` schema 与当前二进制。切换前完成停写、可恢复备份和隔离恢复演练，再由 owner 显式运行 `database-migrate`，随后 role-init 刷新 runtime 最小权限；不得通过手改 `_sqlx_migrations`、放宽 runtime 权限或继续使用 owner URL 绕过这些门禁。2026-07-18 的 v26 role-init 与 LOGIN/TLS 证据保留为历史记录，当前状态以本节 v39 切换记录为准。
...
下面的启动配方**只适用于全新 checkout、全新 Compose project 和空 volume，或已经完成本节全部备份/恢复演练并获得明确升级批准的维护窗口**。当前本机 `mindone` project 已运行 production v39，严禁在其 `deploy/.env`、volume 或默认 project name 上复制全新部署步骤。新部署必须先确认目标 project/volume 从未存在；既有部署必须使用受控升级流程，不能让 `up` 隐式触发未知 migration。
...
> 当前运行门禁：role-init 已把密码、角色属性和全部 ACL/default ACL 刷新放入同一事务。2026-07-23 live v39 实际核对 `mindone_app` 为 LOGIN、NOSUPERUSER、NOCREATEDB、NOCREATEROLE、NOINHERIT、NOREPLICATION、NOBYPASSRLS，且无 TEMP/schema CREATE、成员关系或对象 ownership；0037/0039 新表 ACL 符合最小权限。角色全局连接上限为 32，所有常驻副本和 operator 的连接池总和不得超过该上限。
...
### live v26 到 v39 的已完成升级

上面的 `17 → 26` 是已经完成且应保留的历史事实。升级 v39 前，fresh-v39 已在一次性 PostgreSQL 17 上让 16 个 binary 各用独立数据库通过 `49/49`、无 skip；随后重新为 v26 卷停写并创建升级前备份，在隔离恢复库中演练 `26 → 39`，并验证 0037 邮箱授权、0038 speed class、0039 HMAC-only API Key/只追加事件及最小 ACL。fresh 证据没有被冒充为 live 切换。

2026-07-23 已为当前空闲 live v26 创建权限 `0600` 的 custom-format 备份 `.mindone/backups/production-before-0027-0039-20260722T174308Z/mindone-v26.dump`；文件为 190597 字节，`pg_restore --list` 得到 328 项，SHA-256 为 `2ff4806c4b75723cad508477f7dcc1d396ed021e71e24091ed8e3f7b4e27d451`。备份恢复到独立 PostgreSQL 17 tmpfs 后，关键表行数与源库一致。首次演练发现已部署 migration 14 的文件末尾空行曾被误删；SQL 未变化，但 SQLx checksum 正确拒绝升级。已从本地 Git 历史 blob 恢复部署时的 488 字节原文，并增加固定 checksum 回归，禁止再改写已发布 migration。重建当前 v39 二进制后，恢复副本真实完成 `26 → 39`，metadata 为 `39|1|39|t`，0037 邮箱 token 表、0038 `jobs.speed_class`、0039 API Key/只追加事件表全部存在，`users/jobs/quota_ledger` 仍为 0；一次性容器随后删除，production volume 未被用于演练。

上述第一份备份只完成“备份 + 恢复 + 迁移演练”。实际维护窗口另在停止 writer 后确认 active jobs/prepared routes 为 `0|0`，创建权限 `0600` 的 `.mindone/backups/production-before-0027-0039-live-20260723T141437Z/mindone-v26.dump`；文件为 203480 字节，`pg_restore --list` 得到 381 项，SHA-256 为 `cf4bc43886ec4473ee705720a2f8e112e11c9bdd47e95b1e4a966c510398443f`。

该维护窗口依次完成 owner `database-migrate`、role-init、v39 runtime 与专用 connector 启动。两个 one-shot 退出码均为 `0`；metadata 为 `39|1|39|t`，`mindone_app` 最小权限、0037/0038/0039 对象与 ACL、legacy v1 行 0、`users/jobs/quota_ledger=0|0|0` 均已核对。容器内与公网 `/ready=200` 且身份一致；匿名受保护接口为 401，没有为验收编造账户、额度、账本或网络样本。后续升级必须重新走同样的停写、备份、演练与验收流程。
...
2026-07-23 已在 Cloudflare Dashboard 创建独立 `mindone` remotely-managed Tunnel，并复核唯一已发布应用程序规则为 `api.holarchic.cn`、路径 `*`、服务 `http://coordinator:8787`；Dashboard 同时确认 DNS 记录创建成功。根域以及既有 `aistudio`、`sd` Tunnel 均未修改。专用 token 仅保存在本机 Git/Docker build context 忽略的 `deploy/secrets/cloudflared/token`（`0600`），`.env` 只含该文件路径；双 Compose 静态 `config --quiet` 已通过。受控 `v26→v39` 切换后，完整 overlay 已启动：coordinator 与 connector 均 running/healthy、没有 published ports，并分别只共享 internal `tunnel_edge`；connector 与 coordinator 使用彼此独立的 egress 网络。容器内和公网 MindOne 1.0.2 identity 一致，公网 `/health`、`/ready` 为 200 且带 `CF-Ray`，匿名 `/v1/models`、`/v1/quota/balance` 为 401，未知路径为 404。

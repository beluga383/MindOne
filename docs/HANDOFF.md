...
> 2026-07-23 当前增量：工作树源码与 live production 均为连续 `0001..0039`，65 项 HF 目录、硬件推荐、用户端一键 GGUF 部署、40 个 CLI/TUI 对等动作、三档速度路由、推理 API Key 与 OpenAI JSON/SSE 网关均已完成门禁。TUI 已重做为响应式 Space/Action/Overview/Activity/Command 工作台。v1.0.2 精确提交已通过 Security、完整 CI、Release 重复复验、五平台打包、SBOM/Sigstore/provenance，并已发布为 GitHub latest；五个 provenance 均再次用 `gh attestation verify` 验证。Apple Developer ID/notarization 与 Windows Authenticode 均未配置，不能宣称平台原生签名。live 已在 active jobs/prepared routes 为 `0|0`、二次备份和隔离演练基础上完成 v26→v39；migrator/role-init 均退出 0，runtime ACL 与 `39|1|39|t` 已核对。独立 `mindone` Tunnel、`api.holarchic.cn` 全路径 `*` 到 `http://coordinator:8787` 的路由和专用 connector 均已启用；公网 `/health`、`/ready` 身份/CF-Ray、匿名 401 与未知路径 404 已通过。
...
> 当前结论：源码与 live 均为连续 schema `0001..0039`。当前 workspace `590/0/5`、fresh-v39 `49/49`、API Key 网关 JSON/SSE 数据库 E2E、production 恢复副本 `26→39` 演练和 live 受控切换均已通过。v1.0.2 CLI 与五平台资产已经公开发布；现代 macOS Metal 探测修复已推送到 `main`，对应 Security 全绿，CI 以实时 Actions 终态为准。官方 API 已经通过专用 connector 上线，不能把该公网证据外推为 private catalog、真实 SMTP/TEE、平台原生签名或 Windows 真机模型启动已完成。
...
公网实测显示 `https://holarchic.cn` 的 Cloudflare/TLS 正常，但其路径属于现有官网，不是 MindOne；把根域整站 `/*` 切到协调器会破坏官网。当前合同使用专用子域 `https://api.holarchic.cn`，Base URL 为 `https://api.holarchic.cn/v1`，只在该子域把全路径 `*` 转给 `http://coordinator:8787`。Dashboard 的唯一规则及默认 `http_status:404` catch-all 已复核，根域和既有 `aistudio`/`sd` Tunnel 未修改；live v39 启动后 `/health`、`/ready` 均返回 MindOne 1.0.2 并带 `CF-Ray`，匿名受保护接口为 401。旧客户端配置不会自动覆盖，需显式 `mindone config set server.url https://api.holarchic.cn`。

## 1. 工作树和受保护现场

- 仓库：`/Users/beluga/Documents/MindOne`
- 分支：`codex/complete-cli-mvp-v1`
- 远端发布基线：以 `git rev-parse origin/main` 的实时结果为准；本地分支含本轮 Actions 失败修复和证据文档增量，提交前必须重新核对实际 HEAD。
...
- production `mindone-coordinator-1` 当前健康，使用 v39 镜像和最小权限 `mindone_app`；协调器没有宿主机 published port，production 数据库只读确认是 `39|1|39|t`，PostgreSQL 仍未发布端口。
- Cloudflare 独立 `mindone` Tunnel 路由为 `api.holarchic.cn` 全路径到 `http://coordinator:8787`；token 文件与 `.env` 路径只存在本机 ignored 配置。`mindone-cloudflared-1` running/healthy，固定 digest 且无 published port；不得复用 `18787`、放宽 trusted proxy 或修改既有 Tunnel。
...
- 独立 `mindone serve` 已增加 loopback 管理代理，将 llama.cpp 放在内部随机端口，并强制本机请求使用 slot 0；贡献 worker 只使用 slot 1..3。每个终态推理请求后执行精确 slot erase；请求/响应使用有界通道并尽力 zeroize，清理失败会在下一次推理前失败关闭。受管启动固定注入 `--parallel 4 --kv-unified --slot-save-path`，缺少任一能力均失败关闭。定向门禁已覆盖 all-or-nothing handoff、engine-dead/proxy-live 回收与双贡献槽独立 erase；当前四槽 b10064/GGUF E2E 已在公开 Linux Runner 完成动态推理、逐槽清理与 stop/unpublish。
...
- `model deploy --port`、`serve status --port` 与 `serve stop --port` 使用每端口独立状态、日志和 runtime 目录；`--replace` 只替换同端口实例。`share publish --port` 选择并记录任一健康受管实例，attestation 与 Standard worker 会复验该端口；旧状态缺少字段时兼容为 `8080`。本地仍只有一份活动 share worker/状态，不能借多端口冒充多份容量。多端口状态隔离、模型在用保护和 legacy share state 已有确定性测试。2026-07-23 的 macOS 隔离 E2E 完成同一 Qwen3-0.6B-Q4_0 整包下载、验证、非默认端口 b10064 四槽启动和 `/health`；公开 Linux E2E 随后以当前参数跑完整 Standard 业务链、权威 `log_path` 及轮转代审计和 stop/unpublish。为了遵守小模型/低流量要求，没有下载其他或第二份模型。
- macOS 26 的 `system_profiler` 已把人类文本改为 `Metal Support: Metal 4`，JSON 中提供 `spdisplays_mtlgpufamilysupport=spdisplays_metal4`；旧实现只匹配 `Metal: Supported`，会在 Apple M4 上把推荐后端误报为 CPU。当前实现优先解析严格的 JSON family 版本，保留旧文本回退并拒绝 `unsupported` 伪匹配；本机 `model recommend` 已复验为 `metal=true` / `backend=metal`。
- 当前发行矩阵配置 macOS arm64/x86_64、Linux arm64/x86_64 和 Windows x86_64；llama.cpp release 资产选择有确定性平台映射测试。Windows ARM 没有 MindOne 正式包，安装器会明确拒绝。Linux 两个架构的 CLI release/安装器用户态闭环、Linux 四层沙盒与小模型链，以及 Windows x86_64 原生安装/卸载均已有 Actions 证据；Windows 交互 TUI、Credential Manager、Job Object 生命周期和模型启动仍由用户真机验收。

上述条目是“源码与 v1.0.2 发行资产已有分层证据”，不是“production 已部署”。真实模型证据包括四槽改动前的本机 debug 单 Standard GGUF，以及当前四槽参数的公开 Linux CPU-only Standard 业务链；它仍不能替代 ModelScope 公网、外部 SMTP/浏览器、其他模型/GPU 平台、平台原生签名、真实 TEE 或 private 双 GGUF 证据。
...
| 当前 workspace fmt / check / strict Clippy / tests | 通过 | 31 个 result set，`590 passed / 0 failed / 5 ignored`，退出 0 |
| 当前 CLI/TUI | CLI lib `176/0/1`；40 叶子/10 分类、多端口、平台资产映射通过；macOS Metal 定向 `2/2` | 当前 M4 已按 JSON family 识别为 Metal；回环与真实 macOS capability 用例在未受额外沙箱限制的本机运行；已退出 worker 的回收测试已去除 2 秒调度窗口依赖 |
...
| 独立 serve 请求后清理代理 | proxy 清理定向测试、贡献双槽并发、CPU-only/清理定向测试与 engine strict Clippy 通过 | 当前 workspace 验证强制 slot 0 与逐槽清理；公开 Linux E2E 已以当前四槽/统一 KV 参数完成真实 stop/unpublish |
| Standard SSE / 真实 GGUF E2E | 历史 debug 隔离执行退出 0；当前公开 Linux E2E 完成真实业务链 | fresh PostgreSQL v39、双账号/device、b10064、Qwen3-0.6B-Q4_0、非流式双端点、两类 SSE、游标恢复、密文、三轨唯一结算、策略拒绝零结算、Regulated 流式拒绝和清理均已实际执行；端口感知日志审计由精确候选 Actions 收口 |
| Unix 安装 URL 负例 smoke | 通过 | userinfo/query/fragment/伪 loopback 等拒绝边界 |
| CI/Compose 与外部 Actions | Security 3/3、五目标原生编译、workspace、PostgreSQL、Compose、Unix/Windows 安装、macOS Seatbelt 与 Linux 四层沙盒均已有公开通过证据 | 当前四槽真实 E2E 与端口感知日志审计已跑完整业务链；当前 `main` 的 CI `29988119866` 与 Security `29988121928` 全绿 |
| 当前二进制 release archive smoke | 通过 | `mindone 1.0.2` / `aarch64-apple-darwin`；归档 SHA-256、安装、`--check`、重装、中文 help、doctor JSON、保留数据卸载和 purge 均通过；unsigned local tar.gz |
...
`scripts/e2e-test.sh` 先在四槽改动前的 debug 工作树从头运行并退出 0，随后又在公开 Linux Runner 以当前四槽 argv 和同一小模型完成复验。除 profile provision/replay、OpenAI chat 与 `/v1/completions`、receipt/ledger 和领取后策略变化外，它还确定性完成 public canary worker 终态、两端点 Standard SSE 连续 event ID、唯一 `[DONE]`、AEAD 密文、故障注入后的非零游标恢复、唯一 settlement、逐槽 erase、权威日志审计和清理。恢复测试是代理到 coordinator 的恢复且保持同一 downstream 连接，并不等于公开 API 已提供客户端重新 POST 的 resume token。

## 4. 精确停止点和最可能的首个失败

当前 fresh-v39 `49/49`、workspace `590/0/5`、Actions 全门禁、当前四槽真实 GGUF、五平台打包、v1.0.2 latest Release、checksums、SPDX SBOM、Sigstore bundle 与 GitHub provenance 均已通过并公开。精确停止点是提交独立 Cloudflare Tunnel/DNS 并做公网验收，以及 Windows 真机、ModelScope 公网链、外部邮箱链、private 双 GGUF、真实 TEE、Apple/Windows 平台原生签名和 production live 切换；历史数据库不得重复使用。
...
2026-07-23 又为 live v26 创建并校验 `.mindone/backups/production-before-0027-0039-20260722T174308Z/mindone-v26.dump`，在独立 PostgreSQL 17 tmpfs 恢复后保持 `26|1|26|t` 与零业务行。SQLx 首次拒绝 migration 14 checksum 漂移；取回部署时多一个末尾空行的 488 字节 blob 后 checksum 精确一致，重建二进制在恢复副本完成 `26→39`，0037/0038/0039 结构均已核对。一次性容器已删除，演练备份保留。

同日实际切换前确认 active jobs/prepared routes 为 `0|0`，并创建 `.mindone/backups/production-before-0027-0039-live-20260723T141437Z/mindone-v26.dump`：203480 字节、381 项 TOC、SHA-256 `cf4bc43886ec4473ee705720a2f8e112e11c9bdd47e95b1e4a966c510398443f`。live owner migrator 和 role-init 均退出 0；metadata 为 `39|1|39|t`，0037/0038/0039 结构、runtime 最小权限、零业务行与 legacy 行数 0 均已核对。v39 coordinator 与专用 connector 均 healthy，公网 identity/CF-Ray、匿名 401 和未知路径 404 已通过。
...
### P0-D：真实 GGUF E2E（当前四槽公开 Runner 已通过）
...
- **SLA 可审计排除（migration 0036）**：已实现；历史 fresh-v36/v37 证据之外，当前 fresh-v39 `49/49` 与 workspace `590/0/5` 也已通过。节点或 worker 的 `error_class` 永不自动排除。
...
- **Standard SSE 最终门禁**：实现、runtime role、定向测试、fresh-v39、workspace 与当前四槽公开 GGUF/SSE E2E 已通过；真实执行覆盖密文、chunk 不结算、两端点唯一 final settlement 和同一 downstream 连接中的非零游标故障恢复，不宣称客户端重新 POST resume。
- **独立 serve 请求后清理**：管理代理、all-or-nothing/engine-dead 回收、slot 0 强制绑定、贡献槽独立 erase、全 workspace 与当前四槽真实 b10064/GGUF 终态清理均已通过。只承诺受管 slot erase 和 MindOne 自有缓冲 best-effort 清理，不冒充 GPU 驱动内存物理清零。
...
- **OpenAI 公网网关**：JSON 与 `stream:true` 的真实 PostgreSQL 事务 E2E 均通过；SSE 覆盖连续密文 data、唯一 `[DONE]` 和最终结算。公网 TLS/Cloudflare identity 与匿名拒绝已验证；带真实 API Key 的公网推理、客户端重连续传仍不是本轮公开合同。
...
以下项目仍不能因本轮 live 切换而自行宣称完成：

- production v26 -> v39 已完成；后续 migration 仍须重新执行停写、任务排空、可恢复备份、隔离演练、owner migrator、role-init、runtime 与公网身份验收。
...
- GitHub OAuth、push、PR、Actions、tag、Release、合并、Cloudflare tunnel 和公网切流都需要明确授权与真实外部证据；本轮 push、Tunnel 与公网切流已有授权和证据，不自动授权未来变更。
- 五平台包、SPDX SBOM、Sigstore checksum bundle 与 GitHub provenance 已随 v1.0.2 发布；Apple/Windows 平台原生签名与 macOS notarization 尚无最终证据，公网 TLS/identity 则已通过。

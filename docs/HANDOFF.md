# MindOne v1.0.0 续接任务与行为合同

> 2026-07-22 当前增量：工作树已新增 `0038_job_speed_class.sql` 与 `0039_inference_api_keys.sql`，因此源码目标是连续 `0001..0039`。同时新增 65 项 HF 目录、硬件推荐、64 KiB 不落盘下载探测、用户端一键 GGUF 部署、按端口并行的本地受管实例、40 个 CLI/TUI 对等动作、三档速度路由字段、推理 API Key 与 OpenAI JSON/SSE 网关。TUI 已重做为响应式 Space/Action/Overview/Activity/Command 工作台，包含推荐置顶、可过滤的 65 模型选择器与 `M/R/D/?` 快捷入口；全新客户端默认协调器改为 `https://api.holarchic.cn`，旧配置不覆盖。官方受管进程现固定四槽并显式启用统一 KV：slot 0 只供本机代理，slot 1..3 供真实并发贡献，`max_concurrent=1..3` 可收紧；standard/fast 只用整台空闲贡献端，slow 才在真实容量内装箱。真实 PostgreSQL 回归覆盖 fast 在物理仍有 spare slot 时继续排队、空闲后选高 TPS、slow 聚合与全满排队。最终全量结果已更新到下表。Qwen3-0.6B 公网有界探测只读取 64 KiB。Linux arm64/x86_64 用户态 release 已验证；Windows x86_64 MSVC 已完成交叉 check/strict Clippy/release PE 与静态 CRT/Job Object 导入审计，真机运行/安装仍待 runner。Linux 真实沙盒/模型下载与公网 API 子域 TLS E2E 仍待完成。production 仍是受保护 v26，禁止触碰。
...
> 当前结论：源码现为连续 schema `0001..0039`。当前 workspace `588/0/5`、fresh-v39 `49/49` 和 API Key 网关 JSON/SSE 数据库 E2E 已通过；跨平台真机与公网 TLS 链尚未验收。MindOne v1.0.0 尚未正式发布，也没有授权升级 production。

当前 macOS arm64 又完成一轮仓库外源码安装闭环：`cargo install --locked --path crates/mindone-cli --root <隔离目录>` 成功；只保留隔离 `bin` 与系统目录的最小 PATH 下，`mindone --version`、非交互裸 `mindone` 均退出 0；真实 80×24 PTY 中裸 `mindone` 进入新版工作台，按 `q` 后终端恢复且退出 0。为修复该 smoke 暴露的自动化误报，非 TTY 裸调用现在直接渲染中文根帮助并成功退出，另有二进制合同测试锁定。该证据不代表 GitHub raw/Release 已发布，也不替代 Windows/Linux 原生 runner。
...
此前还用 Debian x86_64 交叉工具链构建 release，并在真实 amd64 Debian 用户态执行版本、中文帮助、硬件推荐、`api info`，再完成相同 SHA-256 安装/`--launch`/重装/卸载/purge 闭环。安装器新增 Unix `--launch` / Windows `-Launch`，可在发布后用单条命令安装并直接进入 TUI，CI/管道会安全降级为帮助页。当前再次匿名复查仍显示仓库、latest Release 与两份 raw 安装器均为 404，`api.holarchic.cn` 的 `/ready`、`/health`、`/v1/models` 仍在严格 TLS 握手阶段失败，因此公网安装/API 尚未上线；Windows 真机、Linux 真实沙盒/模型下载和平台签名仍待外部 runner。
...
- HEAD：`42446c47aa5e48914eb98cfdf416967a342d1e7b`
- 基线提交很小，绝大部分实现仍是 untracked，普通 `git diff` 看不到它们；必须重新运行 `git status --short`、`git ls-files --others --exclude-standard` 和逐路径审阅。交接时仍为 0 staged，不得根据旧的文件数量推断当前集合。
- 禁止 `git add .`、`git clean`、reset、批量覆盖或删除。最终只能逐路径审阅和 stage，三个顶层规格输入默认保留。
- production `mindone-coordinator-1` 当前健康，经 loopback `127.0.0.1:18787` 可达；production 数据库只读确认仍为 `26|1|26|t`。不得把当前 migration、测试或新二进制连接到它。
...
- `model deploy --port`、`serve status --port` 与 `serve stop --port` 使用每端口独立状态、日志和 runtime 目录；`--replace` 只替换同端口实例。多端口状态隔离和模型在用保护已有确定性测试；为了遵守小模型/低流量测试要求，没有额外下载并同时启动第二个真实模型。贡献发布仍只绑定默认 `8080` 实例。
...
| 当前 workspace fmt / check / strict Clippy / tests | 通过 | 31 个 result set，`588 passed / 0 failed / 5 ignored`，退出 0 |
| 当前 CLI/TUI | CLI lib `174/0/1`；40 叶子/10 分类、多端口、平台资产映射通过 | 回环与真实 macOS capability 用例在未受额外沙箱限制的本机运行；已退出 worker 的回收测试已去除 2 秒调度窗口依赖 |
...
| CI/Compose 静态检查 | 当前树已通过 | 当前 16 个 Shell 脚本语法、MVP smoke 合同、邮箱/CLI Web 登录脚本、Compose test config 与 actionlint 均退出 0；真实 Actions 仍待外部运行 |
| 当前二进制 release archive smoke | 通过 | `mindone 1.0.0` / `aarch64-apple-darwin`；归档 SHA-256、安装、`--check`、重装、中文 help、doctor JSON、保留数据卸载和 purge 均通过；unsigned local tar.gz |
...
当前 fresh-v39 `49/49`、workspace `588/0/5`、Linux arm64/x86_64 release 用户态闭环、macOS arm64 隔离源码安装/裸命令 PTY smoke、Windows x86_64 MSVC check/strict Clippy/静态 release PE 审计与 OpenAI JSON/SSE 网关数据库 E2E 已通过。精确停止点是本轮四槽/统一 KV 的小模型真实 E2E、Windows 真机运行/安装器、Linux 真实沙盒/模型下载、公网 API 子域 TLS、ModelScope 公网链、外部邮箱链、private 双 GGUF、真实 TEE、签名发布和受控 production 升级；历史数据库不得重复使用。
...
- **SLA 可审计排除（migration 0036）**：已实现；历史 fresh-v36/v37 证据之外，当前 fresh-v39 `49/49` 与 workspace `588/0/5` 也已通过。节点或 worker 的 `error_class` 永不自动排除。
...
- production v26 -> 当前最终 schema（工作树目标 v39）：需产品/运维 owner 明确维护窗口、停 writer、排空 active jobs/routes、可恢复备份、隔离恢复演练和回滚方案；0034 preflight 通过且 0035..0039 必须全部在恢复副本验证后才能迁移。

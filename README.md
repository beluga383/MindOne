...
> [!IMPORTANT]
> 当前工作树已推进到连续 schema `0001..0039`，但仍处于 v1.0.0 收口阶段，**源码和标签已经公开，GitHub Release 尚未产出，也不是最终全绿状态**。
>
> fresh-v37 的 `43/43`、workspace `556/0/5` 和真实 GGUF E2E 是新增 0038/0039、模型目录与公网网关之前的历史证据。当前树已通过全 workspace check/strict Clippy、全 workspace `589/0/5`，并在一次性 PostgreSQL 17 中以每 binary 独立数据库完成 fresh-v39 `49/49`；API Key 网关的非流式与 Standard SSE 事务 E2E 均已通过。当前源码又在无网络、只读源码的 Linux arm64 Rust 1.88 容器中通过 workspace check、CLI/TUI 测试、release 构建和真实 PTY 启动，macOS Seatbelt allow/deny 门禁也已显式重跑通过；Linux x86_64 release 与安装闭环保留既有真实用户态证据。Windows x86_64 MSVC 已用 `cargo-xwin 0.23.0` 完成全 workspace/all-target/all-feature check、严格 Clippy 和 release 交叉构建，PE 导入审计确认静态 CRT 与 Job Object API；Windows 真机运行/安装器仍待 runner。Qwen3-0.6B 自动部署目标仍只做了 64 KiB 真实有界探测。Linux 真实沙盒/模型下载与公网 TLS E2E 仍待验证。准确停止点见 [v1.0.0 续接文档](docs/HANDOFF.md)。
>
> GitHub 仓库、`main`、`v1.0.0` 标签和 raw 安装器现已公开。首次 Actions 真实运行中，macOS arm64/x86_64 与 Windows x86_64 原生编译通过；整体 Release 因 Linux 条件编译、开发 smoke 对 `rg` 的隐式依赖和两个 PostgreSQL 调度夹具错误而失败，Secret job 则在首次推送时遇到 root commit 父提交不存在的扫描范围边界。当前工作树已修复前三项，并重新通过 Rust 1.88、三平台条件编译、严格 Clippy、workspace `589/0/5`、完整 PostgreSQL CI 序列和本地完整 Git 历史 Gitleaks；修复提交推送和新的正式 Release 仍待完成。
>
...
| 当前 workspace fmt / check / strict Clippy / tests | 通过 | 31 个 result set，`589 passed / 0 failed / 5 ignored`，退出 0；ignored 保留外部资源与平台能力边界 |
...
| 独立 serve 请求后清理代理 | proxy 清理、贡献双槽并发、CPU-only/清理定向测试与 engine strict Clippy 通过 | workspace 验证 slot 0 强制绑定及 slot 1..3 独立清理；最近一次真实 b10064/GGUF E2E 早于四槽/统一 KV 参数变更，发布前须重跑 |
| Standard SSE / 真实 GGUF E2E | 历史 debug 隔离执行通过 | PostgreSQL v37、双账号/device、真实 b10064 + Qwen3-0.6B-Q4_0、非流式双端点、两类 SSE、游标故障恢复、密文、三轨唯一结算、策略拒绝零结算、Regulated 流式拒绝、日志扫描和清理均通过；早于本轮四槽/统一 KV 参数变更，不能冒充当前引擎 argv 的真实证明 |
| Unix 安装 URL 负例 smoke | 通过 | 拒绝凭据、query、fragment、伪 loopback 等 |
| CI/开发 Compose 静态检查 | 当前树通过 | 首次外部 Actions 暴露的 Linux cfg、无 `rg` Runner 与 PostgreSQL 夹具问题已在当前树修复并本地复验；修复提交尚待推送后重跑 |
| 当前二进制 release archive smoke | 通过 | `mindone 1.0.0` / `aarch64-apple-darwin`；验证归档 SHA-256、安装、`--check`、重装、中文 help、doctor JSON、默认保留数据卸载与 purge；仅为 unsigned local smoke |
...
`scripts/e2e-test.sh` 最近一次在四槽改动前的 debug 工作树中从头退出 0：它启动 fresh PostgreSQL v37，建立两个账号/device，安装真实 llama.cpp `b10064` 并验证 `Qwen3-0.6B-Q4_0.gguf`；确定性完成 public canary worker 终态，覆盖 chat 与 `/v1/completions` 非流式、两类 SSE、同一 downstream 连接中的非零游标故障恢复、Standard AEAD ciphertext、消费/节点贡献/网络准备金三轨唯一结算、执行前策略改变拒绝且零结算、Regulated `stream:true` HTTP 400 拒绝、不含 Prompt/Response 明文的日志扫描，以及 stop/unpublish/logout 和本轮资源清理。该恢复断言不表示公开 API 支持客户端重新 POST resume token；本轮只按要求保留小模型下载启动探测，不重新下载完整模型，因此发布前仍须复用小模型重跑当前四槽 argv。
...
2. 修改源码后串行重跑 fmt、workspace check、strict Clippy、workspace tests 和相关 fresh PostgreSQL 门禁；当前基线为 workspace `589/0/5` 与 fresh-v39 `49/49`。
3. 后续修改引擎、worker、代理、SSE 或结算路径时，在不冲突的 loopback 端口重跑真实 GGUF E2E；当前四槽/统一 KV 改动后的这一项仍待复用小模型完成。
...
当前工作树已取得 fmt/check/strict Clippy、workspace `589 passed / 0 failed / 5 ignored` 和 fresh-v39 `49/49`。当前平台 release smoke 与 debug 隔离真实模型 E2E 仍是 0038/0039 前的历史证据；未设置 `DATABASE_URL` 的 skip 不算数据库通过。
...
公开仓库和 raw 安装器已经可匿名读取：[GitHub 仓库](https://github.com/beluga383/MindOne)、[Unix 安装器](https://raw.githubusercontent.com/beluga383/MindOne/main/scripts/install.sh) 和 [Windows 安装器](https://raw.githubusercontent.com/beluga383/MindOne/main/scripts/install.ps1)。`v1.0.0` 标签已经存在，但首次 Release workflow 失败，当前仍没有可下载的 GitHub Release 资产；远程安装器会继续失败关闭，不能把 raw 脚本可达冒充二进制已经发布。完整安装、升级和卸载步骤见 [docs/INSTALL.md](docs/INSTALL.md)。
...
2026-07-22 另在仓库外的隔离安装根执行真实 `cargo install --locked --path crates/mindone-cli --root <临时目录>`：最小 PATH 下裸 `mindone --version` 与非交互裸 `mindone` 均退出 0，真实 80×24 PTY 中裸 `mindone` 进入上述工作台并由 `q` 正常恢复终端、退出 0。该证据只覆盖当前 macOS arm64 源码安装，不替代 Windows/Linux 原生 runner 或尚未发布的远程安装包。
...
同日 Windows x86_64 MSVC 交叉门禁也已补齐：经官方哈希校验的 `cargo-xwin 0.23.0`、Microsoft SDK/CRT 与 `llvm-mingw 20260616` 工具链完成全 workspace/all-target/all-feature check 和 `-D warnings` 严格 Clippy；实际 release 产物为 17 MiB `PE32+ console x86-64`。项目的 `.cargo/config.toml` 对 Windows target 固定启用静态 CRT，导入表不含 `VCRUNTIME140.dll`、`MSVCP*.dll` 或 `api-ms-win-crt-*`，同时确认 `CreateJobObjectW`、`AssignProcessToJobObject`、`SetInformationJobObject` 存在。CI/Release 新增 Windows 原生 `dumpbin` 复验；当前 macOS 无法执行该 EXE，所以 Windows TUI、凭证库、Job Object 生命周期、PowerShell 安装/卸载仍必须由 `windows-latest` 或真机证明。

...
| 当前 workspace fmt / check / strict Clippy / tests | 通过 | 31 个 result set，`588 passed / 0 failed / 5 ignored`，退出 0；ignored 保留外部资源与平台能力边界 |
...
| CI/开发 Compose 静态检查 | 通过 | 尚未在当前本机真实启动 fresh 空卷 |
...
2. 修改源码后串行重跑 fmt、workspace check、strict Clippy、workspace tests 和相关 fresh PostgreSQL 门禁；当前基线为 workspace `588/0/5` 与 fresh-v39 `49/49`。
...
6. 只在明确授权、备份恢复演练和排空后处理 production、GitHub、Cloudflare、Actions、签名、外部 SMTP 和真实 TEE。
...
当前工作树已取得 fmt/check/strict Clippy、workspace `588 passed / 0 failed / 5 ignored` 和 fresh-v39 `49/49`。当前平台 release smoke 与 debug 隔离真实模型 E2E 仍是 0038/0039 前的历史证据；未设置 `DATABASE_URL` 的 skip 不算数据库通过。
...
仓库当前尚未发布 v1.0.0，匿名检查仓库、Release 和 raw 安装器地址目前均为 404；上述 URL 只是预期发布入口，不能冒充已经可用。完整安装、升级和卸载步骤见 [docs/INSTALL.md](docs/INSTALL.md)。

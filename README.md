...
> [!IMPORTANT]
> 当前工作树已推进到连续 schema `0001..0039`。公开 `v1.0.1` 的 Release 专属重复门禁因把 `DATABASE_URL` 注入通用并行 workspace 测试而失败；标签保留且不会移动。修复版 `v1.0.2` 已把通用测试与专用 PostgreSQL 环境隔离，并在精确标签提交上通过完整 CI、安全复验、五平台打包、SBOM/Sigstore/provenance 后发布为 GitHub latest Release。**后续标签仍只允许在同一个精确候选提交的完整 CI 与 Security 全绿后创建**。
>
> fresh-v37 的 `43/43`、workspace `556/0/5` 和真实 GGUF E2E 是新增 0038/0039、模型目录与公网网关之前的历史证据。当前树已通过全 workspace check/strict Clippy、全 workspace `590/0/5`，并在一次性 PostgreSQL 17 中以每 binary 独立数据库完成 fresh-v39 `49/49`；API Key 网关的非流式与 Standard SSE 事务 E2E 均已通过。公开 Actions 已真实通过 macOS arm64/x86_64、Linux x86_64/arm64 与 Windows x86_64 原生编译，以及 Windows 安装/卸载、macOS Seatbelt 和 Linux 四层沙盒门禁；`v1.0.2` 本机归档又通过 SHA-256、安装、更新检查、重装、默认卸载和 purge 闭环。Qwen3-0.6B 自动部署目标日常仍只做 64 KiB 有界探测；同一小模型已在 macOS 隔离 E2E 中完成整包下载、SHA-256/结构验证和非默认端口健康启动，公开 Linux E2E 又以当前四槽参数跑完双账号动态推理、两类 SSE、结算、策略拒绝、权威端口日志审计及清理。v1.0.2 精确标签和随后修复发布上下文的代码基线 `f4adc51` 均取得完整 CI 与 Security 绿色证据。独立 `mindone` Tunnel、`api.holarchic.cn` DNS、origin 与 v39 production overlay 已接通；公网 identity/CF-Ray、匿名 401 和未知路径 404 均已验收。准确状态见 [续接文档](docs/HANDOFF.md)。
>
> GitHub 仓库、`main`、旧 `v1.0.0`/`v1.0.1` 标签、raw 安装器和 [v1.0.2 Release](https://github.com/beluga383/MindOne/releases/tag/v1.0.2) 现已公开；旧标签保留且不会强制移动。v1.0.2 的五平台包、checksums、SPDX SBOM、Sigstore bundle 与 GitHub provenance 已上线。macOS 包未做 Developer ID/notarization，Windows 包未做 Authenticode；具体状态以包内和 Release 的 `SIGNING_STATUS.txt` 为准。
>
> 本机 production 已在排空任务、二次备份和既有隔离演练基础上完成 `v26→v39` 迁移；migrator/role-init 均退出 0，runtime 最小权限与 `39|1|39|t` 已核对。协调器不再发布宿主机端口，只通过专用 Cloudflare connector 暴露 `https://api.holarchic.cn`；PostgreSQL、llama-server 与本地代理端口均未公开。private catalog/HMAC 预算和生产 billing profile 仍需独立配置，不能因 schema 已部署而宣称相应能力已启用。
...
`mindone share stats` 还会使用服务端权威累计贡献值展示固定宽度里程碑进度条；JSON 输出保留整数 microquota、区间和 `progress_ppm`，不混入 ANSI。全网荣誉榜只返回匿名档位和人数下界，不返回 user/device/node/model 标识或精确低样本记录；贡献 cohort 小于 5 时整表抑制，并列节点共享 midrank 档位。协议兼容、协调器纯逻辑、CLI、fresh PostgreSQL、最终 workspace 门禁与四槽确定性回归均已通过；当前四槽真实 GGUF/SSE E2E 也已在公开 Linux Runner 完成。
...
| 独立 serve 请求后清理代理 | proxy 清理、贡献双槽并发、CPU-only/清理定向测试与 engine strict Clippy 通过 | workspace 验证 slot 0 强制绑定及 slot 1..3 独立清理；v1.0.2 与当前 `main` 的公开 Linux E2E 已用当前四槽/统一 KV 参数走到 stop/unpublish、日志审计和清理完成 |
...
| CI/开发 Compose 门禁 | 当前树与公开 Actions 已通过 | production Compose TLS/connector 隔离已在公开 Linux Runner 验证；独立 `mindone` Tunnel、DNS 与 origin 已配置，connector/live v39 与公网请求仍须一起验收 |
...
这些本机 debug、单 Standard GGUF 证据本身不能替代外部门禁；v1.0.2 的 GitHub Actions 与未做平台原生签名的 Release 已另行验证。外部 SMTP/浏览器链、真实 TEE、private hidden 双 GGUF 和 production 部署仍不在这些证据内。
...
production 当前为 v39。数据库 owner 必须显式运行 `mindone-coordinator database-migrate`，常驻应用只使用最小权限 runtime role。后续任何 schema 升级仍必须：
...
当前部署到达 0039 也不等于 private catalog、预算或生产计费 profile 已配置。
...
公开仓库、raw 安装器和 v1.0.2 Release 已可匿名读取：[GitHub 仓库](https://github.com/beluga383/MindOne)、[Unix 安装器](https://raw.githubusercontent.com/beluga383/MindOne/main/scripts/install.sh)、[Windows 安装器](https://raw.githubusercontent.com/beluga383/MindOne/main/scripts/install.ps1) 和 [正式资产](https://github.com/beluga383/MindOne/releases/tag/v1.0.2)。`v1.0.0` 与 `v1.0.1` 失败标签仍保留且不会强制移动；无版本参数的安装器现在解析 latest v1.0.2 并校验 `checksums.sha256`。完整安装、升级和卸载步骤见 [docs/INSTALL.md](docs/INSTALL.md)。
...
2026-07-22 另在仓库外的隔离安装根执行真实 `cargo install --locked --path crates/mindone-cli --root <临时目录>`：最小 PATH 下裸 `mindone --version` 与非交互裸 `mindone` 均退出 0，真实 80×24 PTY 中裸 `mindone` 进入上述工作台并由 `q` 正常恢复终端、退出 0。该证据覆盖当前 macOS arm64 源码安装；Linux/macOS/Windows 的原生 Actions 与 v1.0.2 远程发行资产已有独立外部证据。
